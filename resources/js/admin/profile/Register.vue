<template>
  <div class="flex flex-col gap-y-5 w-2/4 rounded bg-white text-black border-2 border-black text-sm p-10">
    <h3 class="uppercase text-gray-700 font-semibold text-lg">Ты хочешь зарегистрироваться?</h3>
    <div>
      <p class="uppercase">Имя</p>
      <input
  ...
  components:{
    ...
    methods:{
      error(){
        ...
        enterInPapasmoke(){
            axios.post('/api/admin/register', {name: this.name, email: this.login, password: this.password})
            .then(() => {
