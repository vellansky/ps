<section>
    <div class="space-x-2 h-full py-5">
        <h2 class="text-2xl font-semibold tracking-wide cursor-default">{{$category->name}}</h2>
    </div>
    ...
    <div class="flex flex-col gap-4 md:grid md:grid-cols-5">
            ...
            @foreach($category->products as $product)
                <div class="bg-white rounded-xl md:w-full p-2">
                    <a class="flex-none" href="
                        ...
                        <div class="relative h-48">
                            @if($product->new_price)
                                <div class="absolute top-2 right-5 text-xs bg-yellow-200 px-3 rounded">Скидка {{ floor($product->new_price/$product->price * 100) }}%</div>
                            @endif
